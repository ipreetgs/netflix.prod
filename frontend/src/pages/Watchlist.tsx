...
export const Watchlist: React.FC<WatchlistProps> = ({ onOpenModal, myList }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setMovies(res.filter(m => myList.includes(m.id)));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  if (loading) {
    return <div className="p-24 text-center text-neutral-400">Loading Watchlist...</div>;
  }

  return (
    <div className="pt-28 px-12 pb-24">
      <h2 className="text-4xl font-extrabold font-display mb-10">My Watchlist</h2>
      {movies.length === 0 ? (
        <p className="text-neutral-500">Your Watchlist is empty. Browse movies and TV shows and select '+' to add them here.</p>
      ) : (
