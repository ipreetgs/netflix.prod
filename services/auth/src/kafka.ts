...
const clientId = process.env.KAFKA_CLIENT_ID || 'netflix-auth-service';
...
try {
  ...
  console.warn('[Kafka Client] Initialization failed. Kafka features will run in Mock mode.', err);
}
...
export const connectKafka = async () => {
  ...
  try {
    ...
    console.warn('[Kafka] Broker connection failed. Switched to local event simulation mode.', err);
    producer = null;
  }
};
...
export const emitEvent = async (topic: string, payload: any) => {
  ...
  if (producer) {
    try {
      ...
      console.warn(`[Kafka Event Failed] Direct emission failed. Logging fallback...`, err);
    }
  }
